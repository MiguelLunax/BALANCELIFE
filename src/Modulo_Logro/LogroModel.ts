...
export default class LogroModel {
    ...
    public async desbloquearLogro(id_logro: number): Promise<any> {
        ...
        `;
        const checkResult = await Database.executeQuery(checkQuery, [1, id_logro]);
        ...
        if (Array.isArray(checkResult) && checkResult.length > 0) {
            ...
            const updateQuery = `
                ...
                WHERE id_usuario = ? AND id_logro = ?
            `;
            return await Database.executeQuery(updateQuery, [1, id_logro]);
        } else {
            ...
            const insertQuery = `
                ...
                VALUES (?, ?, NOW(), true)
            `;
            return await Database.executeQuery(insertQuery, [1, id_logro]);
        }
    }
    ...
    public async verificarLogrosHidratacion(): Promise<number[]> {
        ...
        `;
        const consecutivosResult = await Database.executeQuery(consecutivosQuery, [1]);
        ...
        const metaDiariaQuery = `
            ...
            WHERE u.id_usuario = ?
        `;
        const metaDiariaResult = await Database.executeQuery(metaDiariaQuery, [1, 1]);
